...
def ingest_curated(curated_dir: Path, corpus_dir: Path, manifest_path: Path) -> CuratedReport:
    ...
    for path in sorted(curated_dir.rglob("*.md")):
        ...
        if collection_for_kind(document_kind, "active") is None:
        ...
        if old and old.file_path:
            ...
            if collision.exists() and collision.read_text(encoding="utf-8") != final:
        ...
        new_metas.append(
            DocMeta(
                ...
                series_key=series_key_for(title, publisher=publisher, scope_unit=scope_unit),
                retention_status="active",
                retention_reason="manual_curated_guide",
