...
另有**冻结的隔离评测集** `data/eval/holdout.v1.jsonl`（47 题，与题库/探针措辞不重复）：
只用于测量、不用于开发调参；2026-09-02 一次性实测 42 答 / 5 拒、要点字面覆盖 94.7%
（`data/coverage/holdout_report.json`），是系统真实泛化能力的参考口径。
