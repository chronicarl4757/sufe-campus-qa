...
在线问答：
  ...
  → 多样性截留(单文档≤3 chunk；同父公告的学院附件同样≤3，点名学院的提问豁免；
    未点名学院时同族模板通知≤2 篇, 防各学院同名年度通知挤占校级制度证据位)
...
另有**冻结的未知措辞（unseen-paraphrase）评测集** `data/eval/holdout.v1.jsonl`
（47 题，与题库/探针措辞不重复）：2026-09-02 一次性实测 **42/47 作答、字面要点覆盖 94.7%**
（`data/coverage/holdout_report.json`）。它反映的是**已覆盖业务上的未知措辞鲁棒性**——
不涉及新业务主题/新学院/多轮澄清/错误前提问题，不代表系统整体正确率。
本集自入库起对开发冻结，禁止按其结果调参；下次正式测量新建 holdout.v2，开发完成后才揭题。

人工金标集 `data/eval/gold.v1.jsonl`（30 题，精标 expected_doc_ids/要点/逐字证据/参考答案）：
标注与维护见 `docs/gold_labeling_guide.md`；`sufe-qa gold-suggest "问题"` 推荐来源候选，
`sufe-qa gold-check` 提交前硬校验（来源现行性 + evidence 逐字在原文中存在）。
